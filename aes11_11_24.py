...
def generate_secure_key(length=12):
    if length < 12:
        raise ValueError("Key length should be at least 12 characters.")
    
    # Định nghĩa các ký tự có thể sử dụng
    lowercase = string.ascii_lowercase  # chữ thường
    uppercase = string.ascii_uppercase  # chữ hoa
    digits = string.digits              # chữ số
    special_characters = string.punctuation  # ký tự đặc biệt

    # Kết hợp tất cả các loại ký tự
    all_characters = lowercase + uppercase + digits + special_characters

    # Đảm bảo key có ít nhất 1 ký tự của mỗi loại
    key = [
    ...
    # Thêm các ký tự ngẫu nhiên vào key cho đến khi đủ độ dài
    key += random.choices(all_characters, k=length - 4)

    # Trộn các ký tự trong key để tăng tính ngẫu nhiên
    random.shuffle(key)
    ...
    def encrypt_file(self, input_path, output_path):
        with open(input_path, 'rb') as infile, open(output_path, 'wb') as outfile:
            while True:
                ...
                self.encrypt(data)
                outfile.write(self.text_encrypt)
...
key = generate_secure_key(16)  # Tạo khóa bảo mật
plain_text = "Hello, this is a test message!"  # Văn bản cần mã hóa
